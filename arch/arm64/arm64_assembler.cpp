...
#define DEBUG_ARM64_INSTRUMENTATION 1

#ifdef DEBUG_ARM64_INSTRUMENTATION
#define IN_RANGE(MSB, LSB, NUM)                                             \
  {                                                                         \
    int32_t __min = ((2 << (MSB - LSB)) / 2) * -1;                          \
    int32_t __max = ((2 << (MSB - LSB)) / 2) - 1;                           \
    if (NUM < __min || NUM > __max)                                         \
      FATAL("number must be in range [%d, %d], was %d", __min, __max, NUM); \
  }
#else
#define IN_RANGE(MSB, LSB, NUM) ;
#endif
...
void Disasm(uint64_t address, size_t num_instr) {
  uint64_t s = address;
  WARN("Disasm at: %llx", address);
  for (size_t i = 0; i < num_instr; ++i) {
    auto instr = arm64::DecodeInstruction((uint64_t)s, *((uint32_t *)s));
    PrintInstruction(s, instr);
    s += 4;
  }
}

// On arm64 the return address should be already stored on the stack,
// hence, we can just load the return address into the lr register
...
void Arm64Assembler::JmpAddress(ModuleInfo *module, size_t address) {
  uint32_t b_instr =
      b(tinyinst_.GetCurrentInstrumentedAddress(module), address);
...
static bool IsReturnInstruction(arm64::Opcode opcode) {
  switch (opcode) {
    case arm64::Opcode::kEret:
    case arm64::Opcode::kEretaa:
    case arm64::Opcode::kEretaaz:
    case arm64::Opcode::kEretab:
    case arm64::Opcode::kEretabz:
...
void Arm64Assembler::FixOffset(ModuleInfo *module, uint32_t jmp_offset,
  ...
  switch (inst.opcode) {
    ...
    case arm64::Opcode::kLdrLiteral:
      IN_RANGE(23, 5, relative_offset);
      encoded_imm = bits(23, 5, relative_offset);
      break;

    case arm64::Opcode::kB:
    case arm64::Opcode::kBl:
      IN_RANGE(23, 0, relative_offset);
      encoded_imm = bits(25, 0, relative_offset);
      break;

    case arm64::Opcode::kTbz:
    case arm64::Opcode::kTbnz:
      IN_RANGE(18, 0, relative_offset);
...
void Arm64Assembler::HandleBasicBlockEnd(
    ...
    const char *code_ptr, size_t offset, size_t last_offset) {
#ifdef DEBUG_ARM64_INSTRUMENTATION
  size_t start_instrumentation = module->instrumented_code_allocated;
#endif
  ...
  if (IsReturnInstruction(inst.instr.opcode)) {
    ...
    PrintInstruction(inst);
    FATAL("shouldn't happen.. I think");
  }
  ...
  PrintInstruction(inst);
  SAY("->\n");
  Disasm((uint64_t)module->instrumented_code_local + start_instrumentation,
         (module->instrumented_code_allocated - start_instrumentation) >> 2);
  SAY("-------------------------------\n\n");
#endif
}
...
bool Arm64Assembler::DecodeInstruction(Instruction &inst,
  ...
  switch (inst.instr.opcode) {
    ...
    case arm64::Opcode::kBrk:
    case arm64::Opcode::kDcps1:
    case arm64::Opcode::kDcps2:
    case arm64::Opcode::kDcps3:
    case arm64::Opcode::kHlt:
    case arm64::Opcode::kHvc:
    case arm64::Opcode::kSmc:
    case arm64::Opcode::kSvc:
      PrintInstruction(inst);
      FATAL("Unhandled instruction");
      break;
