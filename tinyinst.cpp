...
void ModuleInfo::ClearInstrumentation() {
  ...
  for (auto iter = executable_ranges.begin(); iter != executable_ranges.end();
       iter++) {
...
void TinyInst::FixCrossModuleLink(CrossModuleLink *link) {
  ...
  WritePointerAtOffset(module1, translated_value,
                       link->offset1 + child_ptr_size);

  CommitCode(module1, link->offset1, 2 * child_ptr_size);
}

void TinyInst::InvalidateCrossModuleLinks(ModuleInfo *module) {
  for (auto iter = cross_module_links.begin(); iter != cross_module_links.end();
       iter++) {
    if (iter->module2 == module) { InvalidateCrossModuleLink(&(*iter)); }
  }
}
...
void TinyInst::FixCrossModuleLinks(ModuleInfo *module) {
  for (auto iter = cross_module_links.begin(); iter != cross_module_links.end();
       iter++) {
    if (iter->module2 == module) { FixCrossModuleLink(&(*iter)); }
  }
}
...
// When a new indirect jump/call target is detected, this will cause a
// breakpoint which will be resolved by adding a new entry into this hashtable.
void TinyInst::InitGlobalJumptable(ModuleInfo *module) {
  ...
  module->br_indirect_newtarget_global =
      ...
      module->instrumented_code_allocated + JUMPTABLE_SIZE * child_ptr_size +
      child_ptr_size;
  ...
  WritePointer(module, (size_t)module->instrumented_code_remote +
                           module->jumptable_offset);
...
void TinyInst::WritePointer(ModuleInfo *module, size_t value) {
  if (module->instrumented_code_allocated + child_ptr_size >
      module->instrumented_code_size) {
    FATAL("Insufficient memory allocated for instrumented code");
  }
  ...
  if (child_ptr_size == 8) {
    *(uint64_t *)(module->instrumented_code_local +
                  module->instrumented_code_allocated) = (uint64_t)value;
  } else {
    *(uint32_t *)(module->instrumented_code_local +
                  module->instrumented_code_allocated) = (uint32_t)value;
  }
...
// writes a pointer to the instrumented code
void TinyInst::WritePointerAtOffset(ModuleInfo *module, size_t value,
                                    size_t offset) {
...
void TinyInst::FixOffsetOrEnqueue(
    ModuleInfo *module, uint32_t bb, uint32_t jmp_offset,
    std::set<char *>                         *queue,
  ...
  if (iter == module->basic_blocks.end()) {
    ...
    if (queue->find(address) == queue->end()) { queue->insert(address); }
...
bool TinyInst::HandleBreakpoint(void *address) {
  ...
  if (trace_basic_blocks) {
    ...
    if (iter != module->tracepoints.end()) {
      printf(
          "TRACE: Executing basic block, original at %p, instrumented at %p\n",
  ...
  // invalid instruction
  if (module->invalid_instructions.find((size_t)address) !=
      module->invalid_instructions.end()) {
    WARN("Attempting to execute an instruction TinyInst couldn't translate");
    WARN(
        "This could be either due to a bug in the target or the "
        "bug/incompatibility in TinyInst");
    WARN("The target will crash now");
    return true;
  }

  if (unwind_generator->HandleBreakpoint(module, address)) { return true; }
...
bool TinyInst::HandleIndirectJMPBreakpoint(void *address) {
  ...
  if (global_indirect) {
    list_head_offset = module->jumptable_offset + original_address &
  ...
  size_t entry_offset = AddTranslatedJump(
      module, target_module, original_address, translated_address,
      list_head_offset, bp_info, global_indirect);

  size_t continue_address =
      (size_t)module->instrumented_code_remote + entry_offset;

  if (target_module) {
    continue_address = unwind_generator->MaybeRedirectExecution(
        target_module, continue_address);
  }
...
size_t TinyInst::AddTranslatedJump(ModuleInfo *module,
                                   ...
                                   size_t original_target, size_t actual_target,
                                   size_t                 list_head_offset,
  ...
  if (child_ptr_size == 8) {
    previous = (size_t)(*(uint64_t *)(module->instrumented_code_local +
                                      list_head_offset));
  } else {
    previous =
  ...
  assembler_->TranslateJmp(module, target_module, original_target,
                           breakpoint_info, global_indirect, previous_offset);
  ...
  CommitCode(module, entry_offset,
...
TinyInst::IndirectInstrumentation TinyInst::ShouldInstrumentIndirect(
    ModuleInfo *module, Instruction &inst, size_t instruction_address) {
...
void TinyInst::InstrumentIndirect(ModuleInfo *module, Instruction &inst,
  ...
  if (mode == II_GLOBAL) {
    ...
    assembler_->InstrumentLocalIndirect(module, inst, instruction_address,
                                        bb_address);
  } else {
...
void TinyInst::TranslateBasicBlock(
    char *address, ModuleInfo *module, std::set<char *> *queue,
    std::list<std::pair<uint32_t, uint32_t>> *offset_fixes) {
  uint32_t original_offset =
      (uint32_t)((size_t)address - (size_t)(module->min_address));
  uint32_t translated_offset = (uint32_t)module->instrumented_code_allocated;

  unwind_generator->OnBasicBlockStart(module, (size_t)address,
  ...
  if (trace_basic_blocks) {
    ...
    if (GetTargetMethodAddress() == address) { assembler_->Nop(module); }
  }
  ...
  Instruction inst;
  while (true) {
    bool success = assembler_->DecodeInstruction(
        inst, (const unsigned char *)(code_ptr + offset),
        (unsigned int)(code_size - offset));

    if (!success) break;

    unwind_generator->OnInstruction(module, (size_t)address + offset,
                                    GetCurrentInstrumentedAddress(module));
    ...
    InstructionResult instrumentation_result = InstrumentInstruction(
        module, inst, (size_t)address, (size_t)address + offset);
    ...
    switch (instrumentation_result) {
      case INST_HANDLED:
        ...
        continue;
      case INST_STOPBB:
        unwind_generator->OnBasicBlockEnd(
            module, (size_t)address + offset + inst.length,
            GetCurrentInstrumentedAddress(module));
        return;
    ...
    assembler_->FixInstructionAndOutput(
        module, inst, (const unsigned char *)(code_ptr + last_offset),
        (const unsigned char *)(address + last_offset));
  }

  if (!inst.bbend) {
    // WARN("Could not find end of bb at %p.\n", address);
    InvalidInstruction(module);
    unwind_generator->OnBasicBlockEnd(module, (size_t)address + offset,
                                      GetCurrentInstrumentedAddress(module));
    return;
  }

  assembler_->HandleBasicBlockEnd(address, module, queue, offset_fixes, inst,
                                  code_ptr, offset, last_offset);
  unwind_generator->OnBasicBlockEnd(module, (size_t)address + offset,
                                    GetCurrentInstrumentedAddress(module));
}
...
ModuleInfo *TinyInst::GetModule(size_t address) {
  for (auto iter = instrumented_modules.begin();
    ...
    if ((address >= (size_t)cur_module->min_address) &&
      ...
      if (GetRegion(cur_module, address)) { return cur_module; }
    }
  }
...
AddressRange *TinyInst::GetRegion(ModuleInfo *module, size_t address) {
  for (auto iter = module->executable_ranges.begin();
    ...
    if (((size_t)address >= cur_range->from) &&
        ((size_t)address < cur_range->to)) {
      return cur_range;
      break;
    }
  }
...
bool TinyInst::TryExecuteInstrumented(char *address) {
  ...
  translated_address =
      unwind_generator->MaybeRedirectExecution(module, translated_address);
...
void TinyInst::OnReturnAddress(ModuleInfo *module, size_t original_address,
                               size_t translated_address) {
  unwind_generator->OnReturnAddress(module, original_address,
                                    translated_address);
}
...
void TinyInst::InstrumentModule(ModuleInfo *module) {
  ...
  if (persist_instrumentation_data && module->instrumented) {
    ...
    printf(
        "Module %s already instrumented, "
        "reusing instrumentation data\n",
        module->module_name.c_str());
    return;
  }

  ExtractCodeRanges(module->module_header, module->min_address,
                    module->max_address, &module->executable_ranges,
                    &module->code_size);
...
#ifdef ARM64
      (char *)RemoteAllocate(module->instrumented_code_size, READEXECUTE);
#else
      (char *)RemoteAllocateNear((uint64_t)module->min_address,
                                 (uint64_t)module->max_address,
                                 module->instrumented_code_size, READEXECUTE);
#endif
...
void TinyInst::PatchPointersLocal(
    char *buf, size_t size, std::unordered_map<size_t, size_t> &search_replace,
    bool commit_code, ModuleInfo *module) {
  if (child_ptr_size == 4) {
    PatchPointersLocalT<uint32_t>(buf, size, search_replace, commit_code,
                                  module);
  } else {
    PatchPointersLocalT<uint64_t>(buf, size, search_replace, commit_code,
                                  module);
  }
}

template <typename T>
void TinyInst::PatchPointersLocalT(
    char *buf, size_t size, std::unordered_map<size_t, size_t> &search_replace,
    bool commit_code, ModuleInfo *module) {
  ...
  for (size_t i = 0; i < size; i++) {
    ...
    if (iter != search_replace.end()) {
      ...
      // (size_t)buf); if (commit_code) printf("The address is in translated
      // code\n");
      T fixed_ptr = (T)iter->second;
      *(T *)(buf) = fixed_ptr;
      if (commit_code) { CommitCode(module, i, child_ptr_size); }
    }
    buf += 1;
  }
}
...
void TinyInst::PatchModuleEntries(ModuleInfo *module) {
  ...
  for (size_t handler : exception_handlers) {
    ...
    if (iter != search_replace.end()) {
      // printf("Removing exception handler %zx from list of entrypoints to
      // patch\n", handler);
  ...
  if (patch_module_entries & PatchModuleEntriesValue::DATA) {
    PatchPointersRemote(module->min_address, module->max_address,
                        search_replace);
  }
  ...
  if (patch_module_entries & PatchModuleEntriesValue::CODE) {
    ...
    for (AddressRange &range : module->executable_ranges) {
      PatchPointersLocal(range.data, (range.to - range.from), search_replace,
                         false, NULL);
    }
    // some of that code could have been translated already
    // while translating entrypoints themselves
    PatchPointersLocal(module->instrumented_code_local,
                       module->instrumented_code_allocated, search_replace,
                       true, module);
  }
}
...
void TinyInst::OnInstrumentModuleLoaded(void       *module,
                                        ModuleInfo *target_module) {
  if (target_module->instrumented && target_module->module_header &&
      (target_module->module_header != (void *)module)) {
    WARN(
        "Instrumented module loaded on a different address than seen "
        "previously\n"
        "Module will need to be re-instrumented. Expect a drop in "
        "performance.");
    ClearInstrumentation(target_module);
  }

  target_module->module_header = (void *)module;
  GetImageSize(target_module->module_header, &target_module->min_address,
...
void TinyInst::OnModuleUnloaded(void *module) {
  ...
  for (auto iter = instrumented_modules.begin();
    ...
    if (cur_module->module_header == (void *)module) {
      ...
      if (!persist_instrumentation_data) { ClearInstrumentation(cur_module); }
...
void TinyInst::OnTargetMethodReached() {
  ...
  if (target_function_defined && !instrument_modules_on_load)
...
bool TinyInst::OnException(Exception *exception_record) {
  switch (exception_record->type) {
    case BREAKPOINT:
      if (HandleBreakpoint(exception_record->ip)) { return true; }
...
void TinyInst::Init(int argc, char **argv) {
  ...
  instrument_modules_on_load =
      GetBinaryOption("-instrument_modules_on_load", argc, argv, false);
  patch_return_addresses =
      GetBinaryOption("-patch_return_addresses", argc, argv, false);
  instrument_cross_module_calls =
      GetBinaryOption("-instrument_cross_module_calls", argc, argv, true);
  persist_instrumentation_data =
      GetBinaryOption("-persist_instrumentation_data", argc, argv, true);

  trace_basic_blocks =
      GetBinaryOption("-trace_basic_blocks", argc, argv, false);
  trace_module_entries =
      GetBinaryOption("-trace_module_entries", argc, argv, false);

  full_address_map = GetBinaryOption("-full_address_map", argc, argv, false);

#if defined(ARM64) && defined(__APPLE__)
  page_extend_modules =
      GetBinaryOption("-page_extend_modules", argc, argv, true);
#else
  ...
  if (page_extend_modules) {
    ...
    if (mod_grp.empty()) { FATAL("Module group is expected to have entries."); }
